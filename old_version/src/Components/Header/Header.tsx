...
export default function Header({
  ...
  return (
    <div className="flex-col">
      <Collapsible open trigger={"Christmas Bird Count Compiler"}>
      <div className="flex-1">
        ...
        notes. Select the &lsquo;group&rsquo; status for each point to group
        overlapping points together.<br></br>
        4.) Species Mode allows you to go species by species when grouping
        checklists. <br></br>
        5.) Click &lsquo;&lsquo;Get Species&rsquo;&rsquo; to get the species
        from the database and compile them into a CSV file that will download.
        <br></br>
      </div>
      </Collapsible>
      <div className="flex-1">
      ...
      <div className="flex-1">
        ...
        <input
          type="checkbox"
          checked={speciesMode}
          onChange={() => setSpeciesMode(!speciesMode)}
        />
        <label>Species Mode</label>
        {speciesMode && (
          <p>
            Total for {speciesForView}: {species[speciesForView]}
          </p>
        )}
      </div>
    </div>
